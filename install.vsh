...
const analyzer_config_dir_path = join_path(home_dir(), '.config', 'velvet')
...
const analyzer_bin_file_path = join_path(analyzer_bin_dir_path, 'velvet')
...
fn current_version() !string {
	version_res := os.execute('velvet --version')
	if version_res.exit_code != 0 {
		return error('Failed to get current version: ${version_res.output}')
	}

	return version_res.output.trim_string_left('velvet version ').trim_space()
}
...
fn check_updates(release_type string) ! {
	...
	if local_version == asset_version {
		println('You already have the latest version of ${term.bold('velvet')}: ${cur_version}')
		return
	}
	println('New version of ${term.bold('velvet')} is available: ${term.bold(asset.tag_name)}')
}
...
fn install(no_interaction bool, release_type string) ! {
	println('Downloading ${term.bold('velvet')}...')
	...
	println('Found ${term.bold('velvet')} binary for your platform: ${asset.os_arch()}')
...
fn install_from_binary(asset ReleaseAsset, update bool) ! {
	...
	print('Downloading ${term.bold('velvet')} archive')
	os.flush()

	archive_temp_dir := os.join_path(os.temp_dir(), 'velvet', 'archive')
	os.mkdir_all(archive_temp_dir) or {
		println('Failed to create temp directory for archive: ${archive_temp_dir}')
		return
	}

	archive_temp_path := os.join_path(archive_temp_dir, 'velvet.zip')

	download_file_with_progress(asset.browser_download_url, archive_temp_path)

	println('${term.green('✓')} Successfully downloaded ${term.bold('velvet')} archive')

	println('Extracting ${term.bold('velvet')} archive...')
	...
	println('${term.green('✓')} Successfully extracted ${term.bold('velvet')} archive')

	if update {
		println('${term.green('✓')} ${term.bold('velvet')} successfully updated to ${term.bold(asset.tag_name)}')
	}
...
fn find_latest_asset(release_type string) !ReleaseAsset {
	text := http.get_text('https://api.github.com/repos/DaZhi-the-Revelator/velvet/releases/latest')
...
fn update_from_sources(update bool, nightly bool) ! {
	...
	if need_pull {
		println('Updating ${term.bold('velvet')} sources...')
		...
		println('${term.green('✓')} Successfully updated ${term.bold('velvet')} sources')
	}
	...
	if update {
		...
		println('${term.green('✓')} ${term.bold('velvet')} successfully updated to ${updated_version}')
	}
...
fn clone_repository() ! {
	println('Cloning ${term.bold('velvet')} repository...')

	exit_code := run_command('git clone ${git_clone_options} https://github.com/DaZhi-the-Revelator/velvet.git ${analyzer_sources_dir_path} 2>&1') or {
		errorln('Failed to clone velvet repository: ${err}')
		return
	}
	if exit_code != 0 {
		errorln('Failed to clone velvet repository')
		return
	}

	println('${term.green('✓')} ${term.bold('velvet')} repository cloned successfully')
}

fn build_from_sources() ! {
	println('Building ${term.bold('velvet')}...')

	chdir(analyzer_sources_dir_path)!
	install_deps_cmd := os.execute('v install')
	if install_deps_cmd.exit_code != 0 {
		errorln('Failed to install dependencies for ${term.bold('velvet')}')
		eprintln(install_deps_cmd.output)
		return
	}

	println('${term.green('✓')} Dependencies for ${term.bold('velvet')} installed successfully')

	chdir(analyzer_sources_dir_path)!
	exit_code := run_command('v build.vsh 1>/dev/null') or {
		errorln('Failed to build ${term.bold('velvet')}: ${err}')
		return
	}
	if exit_code != 0 {
		errorln('Failed to build ${term.bold('velvet')}')
		return
	}

	println('Moving ${term.bold('velvet')} binary to the standard location...')
	...
	os.cp_all('${analyzer_sources_dir_path}/bin/velvet' + $if windows { '.exe' } $else { '' },
		analyzer_bin_dir_path, true) or {
		println('Failed to copy ${term.bold('velvet')} binary to ${analyzer_bin_dir_path}: ${err}')
		return
	}

	println('${term.green('✓')} Successfully moved ${term.bold('velvet')} binary to ${analyzer_bin_dir_path}')

	println('${term.green('✓')} ${term.bold('velvet')} built successfully')
}
...
fn show_hint_about_path_if_needed(abs_path string) {
	...
	println('${term.bold('{')}')
	println('    ${term.yellow('"velvet.serverPath"')}: ${term.green(quoted_abs_path)}')
	println('${term.bold('}')}')
}
...
fn main() {
	...
	mut cmd := cli.Command{
		name:        'velvet-installer'
		version:     installer_version
		description: 'Install and update velvet'
		posix_mode:  true
	...
	cmd.add_command(cli.Command{
		name:        'up'
		description: 'Update velvet to the latest version'
		posix_mode:  true
		execute:     fn (cmd cli.Command) ! {
	...
	cmd.add_command(cli.Command{
		...
		description: 'Check if velvet binary is available for the current platform (service command for editors)'
		posix_mode:  true
		execute:     fn (cmd cli.Command) ! {
			release_type := get_release_type(cmd)
			find_latest_asset(release_type) or {
				println('Prebuild velvet binary is not available for your platform')
				return
			}

			println('${term.green('✓')} Prebuild velvet binary is available for your platform')
		}
	})
