...
pub fn (mut v InlayHintsVisitor) accept(root psi.PsiElement) {
	...
	// Filter results to the requested range if one was provided
	if v.range_start >= 0 && v.range_end >= 0 {
		v.result = v.result.filter(it.position.line >= v.range_start
			&& it.position.line <= v.range_end)
	}
}
...
pub fn (mut v InlayHintsVisitor) process_node(node psi.AstNode, containing_file &psi.PsiFile) {
	...
	if node.type_name == .or_block_expression && v.cfg.enable_implicit_err_hints {
		expression_node := node.first_child() or {
			loglib.info('[hints] or_block_expression: first_child() returned none')
			return
		}
		expression := psi.create_element(expression_node, containing_file)
		typ := psi.infer_type(expression)
		loglib.info('[hints] or_block_expression: inferred type = ${typ}')
	...
	if node.type_name == .else_branch && v.cfg.enable_implicit_err_hints {
		loglib.info('[hints] else_branch: calling handle_if_unwrapping')
...
pub fn (mut v InlayHintsVisitor) handle_call_expression(call psi.AstNode, containing_file &psi.PsiFile) {
	...
	if call_expression is psi.CallExpression {
		...
		called := call_expression.resolve() or {
			loglib.info('[hints] handle_call_expression: resolve() returned none for call at line ${call.start_point().row}')
			return
		}
		if called is psi.SignatureOwner {
			signature := called.signature() or {
				loglib.info('[hints] handle_call_expression: signature() returned none')
				return
			}
...
pub fn (mut v InlayHintsVisitor) handle_if_unwrapping(node psi.AstNode, containing_file &psi.PsiFile) {
	parent_if_expression := node.parent_of_type(.if_expression) or {
		loglib.info('[hints] handle_if_unwrapping: parent_of_type(.if_expression) returned none')
		return
	}

	guard := parent_if_expression.child_by_field_name('guard') or {
		loglib.info('[hints] handle_if_unwrapping: no guard field')
		return
	}
	expression_list := guard.child_by_field_name('expression_list') or {
		loglib.info('[hints] handle_if_unwrapping: no expression_list field')
		return
	}
	expression := expression_list.first_child() or {
		loglib.info('[hints] handle_if_unwrapping: expression_list has no children')
		return
	}
	expr_type := psi.infer_type(psi.create_element(expression, containing_file))
	loglib.info('[hints] handle_if_unwrapping: inferred type = ${expr_type}')
