...
fn (mut p Provider) struct_documentation(element psi.StructDeclaration) ? {
	...
	for field in element.own_fields() {
		if field is psi.FieldDeclaration {
			...
			is_mut, is_pub := field_decl.is_mutable_public()
			scope := if is_pub && is_mut {
				'pub mut'
			} else if is_pub {
				'pub'
			} else if is_mut {
			...
			if scope != last_scope {
				if scope != '' {
					p.sb.write_string('\t${scope}:\n')
				}
				last_scope = scope
			}
			// Use get_text() for the full field declaration — it is always
			// safe for both tree-based and stub-based PSI elements, whereas
			// calling get_type() on stub fields can panic during type
			// inference when the underlying file has been freed.
...
fn (mut p Provider) enum_documentation(element psi.EnumDeclaration) ? {
	...
	for field in element.fields() {
		if field is psi.EnumFieldDeclaration {
			field_decl := field as &psi.EnumFieldDeclaration
			p.sb.write_string('\t${field_decl.name()} = ${field_decl.value_presentation(true)}\n')
		}
	}
	p.sb.write_string('}\n')
	p.sb.write_string('```')
