...
pub fn (mut ls LanguageServer) initialize(params lsp.InitializeParams, mut wr ResponseWriter) lsp.InitializeResult {
	...
	ls.setup()
	ls.apply_initialization_options(params.initialization_options)
	...
	ls.register_compiler_quick_fix(intentions.RemoveUnusedImportQuickFix{})
	...
	return lsp.InitializeResult{
		capabilities: lsp.ServerCapabilities{
			...
			code_action_provider:         lsp.CodeActionOptions{
				code_action_kinds: [lsp.quick_fix, lsp.refactor]
			}
...
fn (mut ls LanguageServer) apply_initialization_options(options lsp.LSPAny) {
	if options is map[string]lsp.LSPAny {
		...
		if inlay_hints_any is map[string]lsp.LSPAny {
			...
			if enable_any := ih['enable'] {
				if enable_any is bool {
					ls.cfg.inlay_hints.enable = enable_any
				}
			}
			if v := ih['enable_type_hints'] {
				if v is bool {
					ls.cfg.inlay_hints.enable_type_hints = v
				}
			}
			if v := ih['enable_parameter_name_hints'] {
				if v is bool {
					ls.cfg.inlay_hints.enable_parameter_name_hints = v
				}
			}
			if v := ih['enable_range_hints'] {
				if v is bool {
					ls.cfg.inlay_hints.enable_range_hints = v
				}
			}
			if v := ih['enable_implicit_err_hints'] {
				if v is bool {
					ls.cfg.inlay_hints.enable_implicit_err_hints = v
				}
			}
			if v := ih['enable_constant_type_hints'] {
				if v is bool {
					ls.cfg.inlay_hints.enable_constant_type_hints = v
				}
			}
			if v := ih['enable_enum_field_value_hints'] {
				if v is bool {
					ls.cfg.inlay_hints.enable_enum_field_value_hints = v
				}
			}
