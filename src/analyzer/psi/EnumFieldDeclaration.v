...
fn (f &EnumFieldDeclaration) get_value_impl() i64 {
	...
	if prev_field is EnumFieldDeclaration {
		prev_decl := prev_field as &EnumFieldDeclaration
		prev_value := prev_decl.get_value()
		if is_flag {
			return prev_value * 2
		}
		return prev_value + 1
...
fn (f &EnumFieldDeclaration) calculate_value(value PsiElement) ?i64 {
	if value is Literal {
		lit := value as &Literal
		return lit.value()
	}

	if value is UnaryExpression {
		unary := value as &UnaryExpression
		if unary.operator() == '-' {
			if expr := unary.expression() {
	...
	if value is BinaryExpression {
		binary := value as &BinaryExpression
