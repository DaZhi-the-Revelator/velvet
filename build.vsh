...
const vexe = @VEXE
const bin_path = './bin/velvet' + $if windows { '.exe' } $else { '' }
...
fn build(mode ReleaseMode, explicit_debug bool) {
	...
	println('${ynote} Building velvet at commit: ${build_commit} .')
	println('${ynote} Building start time: ${build_datetime} .')

	cmd := mode.compile_cmd()
	println('${ynote} Compiling velvet in ${term.bold(mode.str())} mode, using:')
	println(cmd)
	if mode == .release {
	...
	println('${gcheck} Successfully built velvet, in ${elapsed_ms / 1000.0:5.3f}s .')
}
...
mut cmd := cli.Command{
	...
	description: 'Builds the velvet binary.'
	posix_mode:  true
